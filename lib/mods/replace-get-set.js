...
function replaceGlobalGetSetCalls(node, imports) {
  ...
  if (isGlobalPropertyAccess(node)) { // global get
  ...
  } else if (isWindowGlobal(node)) { // non-property access window get-set
    const parent = node.getParent();

    if (isSet(parent)) { // non-property access window set
      type = 'set';
      nodeToReplace = parent;
      args = parent.getRight().getText();
    } else { // non-property access window get
      type = 'get';
      nodeToReplace = node;
    }
    ...
    return;
  } else { // not a get-set - move on to next node
    return;
  }

  // continue for normal get-set
