...
function isGlobalPropertyAccess(node) {
  // Checks if the parent is a call expression and
  // the node is its expression (not one of its arguments)
  const isCallExpressionChild = nodeTypes.isCallExpression(node.getParent()) &&
      node.getParent().getExpression() === node;

  return nodeTypes.isPropertyAccessExpression(node) &&
    !isCallExpressionChild &&
...
function replaceGlobalGetSetCalls(node, imports) {
  ...
  if (type === 'window-get' || type === 'window-set') {
    ...
    moduleName = getModuleNameForPropertyOfNode(base, propertyName);
    baseObject = base.getText();

    // If a node has call signatures, it's a function
    if (nodeToInspect.getType().getApparentType()
        .getCallSignatures().length !== 0) {
      if (type === 'set' || isSet(node.getParent())) {
        return;
      }
      type = 'fn-' + type;
    }
