...
function isWindowGlobal(node) {
  ...
  // filter function instance data properties
  if (symbol.compilerSymbol.parent &&
    symbol.compilerSymbol.parent.getName() === 'Function') {
    return false;
  }
  ...
  if (nodeTypes.isPropertyAccessExpression(parent)) {
    ...
    if (symbol.compilerSymbol.parent) {
      const baseName = symbol.compilerSymbol.parent.getName();
