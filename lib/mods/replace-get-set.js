...
/**
 * Checks if node in question is a non-property access window
 * identifier, by eliminating other possible types of globals
...
function isWindowGlobal(node) {
  ...
  if (nodeTypes.isPropertyAccessExpression(parent)) {
    baseName = parent.getExpression().getType().getSymbol().getName();
