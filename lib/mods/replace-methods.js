...
/**
 * Builds a `call` function from the components of a method call
 *
 * @param {*} base - base upon which to call method
 * @param {*} original - original method to call
 * @param {*} args - arguments to the method
 *
 * @return {string} the represented call expression
 */
function formatCall(base, original, args) {
  return `call(${base}, ${original}, [${args}])`;
}
...
function replaceGlobalMethodCalls(node, imports) {
  if (nodeTypes.isPropertyAccessExpression(node) &&
    ...
    node.getParent().replaceWithText(
        formatCall(callee, original, args));
    updateImports(callerType, method, 'method', imports);
  }
}
