 ...
 * @param {*} baseTypeName -
 *
 * @return {*} whether the method is a namespace method
 */
function isNamespaceMethod(type, node, baseTypeName) {
...
function replaceGlobalMethodCalls(node, imports) {
  if (nodeTypes.isPropertyAccessExpression(node) &&
    ...
    if (type !== 'static' && isNamespaceMethod(type, node, baseTypeName)) {
      type = 'namespace';
      callee = baseTypeName;
    }
