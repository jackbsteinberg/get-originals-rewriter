 ...
 */
function isWindowFunction(node) {
  const symbol = node.getExpression().getSymbol();
  const firstDeclaration = symbol.getDeclarations()[0];

  // TODO: may require a deeper check
  // e.g. setTimeout web returns number, Node returns Timer
  if (firstDeclaration) {
    return window.hasOwnProperty(firstDeclaration.getName());
  }
  return window.hasOwnProperty(symbol.getName());
}
...
function replaceWindowFunctions(node, imports) {
  if (nodeTypes.isCallExpression(node) && isWindowFunction(node)) {
    const symbol = node.getExpression().getSymbol();
    const firstDeclaration = symbol.getDeclarations()[0];
    const type = 'window';
    const mod = 'Window';
