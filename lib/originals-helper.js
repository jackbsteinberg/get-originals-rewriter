...
/**
 * @param {Node} node - a ts-morph AST node
 * @param {string} property - the property to find on some module
 *
 * @return {string} the get-originals module name corresponding to the node's
 * type. (This is sometimes different from the TypeScript type name.)
 */
function getModuleNameForNodeFromPrototypeChain(node, property) {
  return getModuleNameForTypeFromPrototypeChain(node.getType(), property);
}
 ...
 * @param {string} property - the property to find on some module
 *
 * @return {string} whether the property exists on the given type
 */
function checkPropertyOnType(type, property) {
  for (const symbol of type.getProperties()) {
    if (property === symbol.getName()) {
      return true;
    }
  }
  return false;
 ...
 * type. (This is sometimes different from the TypeScript type name.)
 */
function getModuleNameForTypeFromPrototypeChain(type, property) {
  // Need to use BaseTypeOfLiteralType to catch the type of primitives
  switch (type.getBaseTypeOfLiteralType().getText()) {
    case 'string': {
      return 'String';
    }
    case 'number': {
  ...
  while (inspectType !== undefined) {
    if (checkPropertyOnType(inspectType, property)) {
      latestFound = inspectType;
    } else if (latestFound !== inspectType) {
      return latestFound.getSymbol().getName();
    }

    inspectType = inspectType.getSymbol()
        .getDeclarations()[0].getBaseTypes()[0];
  }
